...
const githubContext = config.input.runInOrgRunner ? { owner: config.githubContext.owner } : config.githubContext;
...
async function getRunner(label) {
  ...
  try {
    const runners = await octokit.paginate(`GET ${runnerBasePath}/actions/runners`, githubContext);
...
async function getRegistrationToken() {
  ...
  try {
    const response = await octokit.request(`POST ${runnerBasePath}/actions/runners/registration-token`, githubContext);
...
async function removeRunner() {
  ...
  try {
    await octokit.request(`DELETE ${runnerBasePath}/actions/runners/{runner_id}`, _.merge(githubContext, { runner_id: runner.id }));
